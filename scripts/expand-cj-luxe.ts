...
const BANNED = new Set(["2095813724408475650"]); // wig that slipped allow-filter
const existingClean = existing.filter((p: Product) => !BANNED.has(p.cjPid));
if (existingClean.length !== existing.length) console.log(`removed banned: ${existing.length - existingClean.length}`);
const seen = new Map<string, Raw>();
for (const p of existingClean) {
  seen.set(p.cjPid, {
...
const existingIds = new Set(existingClean.map((p) => p.cjPid));
const fresh: Product[] = [...seen.values()]
  .filter((c) => !existingIds.has(c.pid) && !BANNED.has(c.pid))
  .map(toProduct);
...
const merged = [...existingClean, ...fresh].slice(0, CAP);
...
const stats = {
  ...
  existingKept: existingClean.length,
  newAdded: merged.length - existingClean.length,
  apiCalls,
  apiErrors,
  ...
  source: "CJ /product/list via productName param, luxe pass 2026-09-08",
};
